...
export default function InstructionDetailCard({id,data}) {
  ...
  async function deleteUser(id){
     firebase.firestore().collection('inst').doc(id).delete().then(alert("dleted")).catch((error)=>alert(error))
    ...
    <Card style={{
      ...
      <span>Title</span>
        ...
        { data.answers && data.answers.map((e)=>{
          return(<div >
            <Typography style={{marginLeft:5}} variant="body2" color="textPrimary" component="p">
          {e.name}
        </Typography>
      </div>)
        })}
