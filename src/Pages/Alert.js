...
import InstrcutionDetailTable from "../components/InstructionDetailTable";
import AdminTable from "../components/AdminTable";
import { LinearProgress, withStyles } from "@material-ui/core";
import AuthContext from "../components/config/context";
import AlertDetailTable from "../components/AlertDetailTable";
...
export default function Alert() {
  ...
  async function countEx() {
    let i = 0 
    let y = 0
    const exRef = await firebase.firestore().collection('alert').get()
  ...
  const fixedHeightPaper = clsx(classes.paper, classes.fixedHeight);
      ...
      async function countVote(){
        console.log("yr b ")
        const voteRef = await firebase.firestore().collection('alert').get()
...
<Link style={{cursor:'pointer' ,textDecoration:"none", marginLeft:40, padding:10, borderRadius:5, backgroundColor:"#3F51B5", color:'white',  }} href = '/NewAlert'>New Alert</Link>
</Grid>
      ...
      {votelist && <Grid item xs={12}>
              <Paper className={classes.paper}>
                <AlertDetailTable data={votelist}  />
              </Paper>
            </Grid>}
