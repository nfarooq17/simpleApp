...
export default function NewVotes() {
  ...
  async function addVote(value){
    ...
    let arr=[]
      ...
      value.options=arr
    await firebase.firestore().collection('votes').add(value)
     alert ('Done!')
      ...
      {({ submitForm, isSubmitting }) => (
        <Form className={classes.form} noValidate>
          ...
          {positions.map((name) => (
            ...
            <Button style={{backgroundColor:'#3F51B5', color:'white'}} onClick={()=>addCount()}>Add Field</Button>
            {count>=1 &&<Button style={{backgroundColor:'#FF0000', color:'white', marginLeft:10 }} onClick={()=>deleteCount()}>Add Field</Button>}
            </div>
